...
def get_factor_decomposition(univ_factor_df, topN_weight, all_infocode_roc_monthly, trading_cost, rebalancing_freq = 'm'):
    ...
    amount_begin = 100
    for j,i in enumerate(topN_weight.index):
        if i == topN_weight.index[0]:
            end_weight_df.loc[i] = begin_weight_df.loc[i]
            begin_amount_df.loc[i] = begin_weight_df.loc[i] * amount_begin
            holding_end_amount_df.loc[i] = amount_begin * end_weight_df.loc[i]
        else:
            holding_weight_end = begin_weight_df.iloc[j-1] * (all_infocode_roc_monthly.loc[i] + 1)
    ...
    holding_end_amount_pl = pd.melt(holding_end_amount_pl, id_vars=['date_'], value_vars=holding_end_amount_pl.columns.tolist()[1:])
    ...
    univ_factor_concat = pl.concat([univ_factor_df_unpivot,begin_weight_pl,begin_amount_pl,end_weight_pl,holding_end_amount_pl])
    univ_factor_df = univ_factor_concat.pivot(values='value',index=['date_','infocode'],columns='variable')
    univ_factor_df = univ_factor_df.with_columns([pl.col(["RI","begin_weight","begin_amount","end_weight","holding_end_amount","Price/Earnings Ratio - Current","short_MA",'Market Capitalization - Current (U.S.$)']).cast(pl.Float64, strict=False)])
    ...
    for i in topN_weight.index:
        ...
        univ_factor_df_i['Price/Earnings Ratio - Current'][univ_factor_df_i['Price/Earnings Ratio - Current']<0] = univ_factor_df_i['Price/Earnings Ratio - Current'].median()
        ...
        if len(i_rebal_before_holdings) > 0:
            ...
            i_rebal_before_holdings['shares'] = i_rebal_before_holdings['holding_end_amount'] / i_rebal_before_holdings['RI']
            i_rebal_before_holdings['ma_shares'] = i_rebal_before_holdings['holding_end_amount'] / i_rebal_before_holdings['short_MA']
            i_rebal_before_holdings['EARNINGS'] = i_rebal_before_holdings['Market Capitalization - Current (U.S.$)'] / i_rebal_before_holdings['Price/Earnings Ratio - Current']
            i_rebal_before_holdings['total_shares'] = i_rebal_before_holdings['Market Capitalization - Current (U.S.$)'] / i_rebal_before_holdings['RI']
            i_rebal_before_holdings['EPS'] = i_rebal_before_holdings['RI'] * (1/i_rebal_before_holdings['Price/Earnings Ratio - Current'])
            i_rebal_before_holdings['MA_EPS'] = i_rebal_before_holdings['short_MA'] * (1/i_rebal_before_holdings['Price/Earnings Ratio - Current'])
            i_rebal_before_holdings['PER_'] = i_rebal_before_holdings['Market Capitalization - Current (U.S.$)'] * (1/i_rebal_before_holdings['EARNINGS'])
        ...
        i_holdings['EARNINGS'] = i_holdings['Market Capitalization - Current (U.S.$)'] / i_holdings['Price/Earnings Ratio - Current']
    ...
    decomposition_ts_df['holding_NOISE_change'] = decomposition_ts_df['holding_i_NOISE'] / decomposition_ts_df['i_NOISE'].shift() - 1
    decomposition_ts_df['holding_EARNINGS_change'] = decomposition_ts_df['holding_i_EPS'] / decomposition_ts_df['EPS_shift'] - 1
    decomposition_ts_df['holding_MA_EARNINGS_change'] = decomposition_ts_df['holding_i_MA_EPS'] / decomposition_ts_df['i_MA_EPS'].shift() - 1
    decomposition_ts_df['holding_PER_change'] = decomposition_ts_df['holding_i_PER'] / decomposition_ts_df['PER'].shift() - 1
    decomposition_ts_df['holding_MA_PER_change'] = decomposition_ts_df['holding_i_MA_PER'] / decomposition_ts_df['MA_PER'].shift() - 1
    decomposition_ts_df['rebalancing_EARNINGS_change'] = decomposition_ts_df['i_EPS'] / decomposition_ts_df['holding_i_EPS'] - 1
    decomposition_ts_df['rebalancing_MA_EARNINGS_change'] = decomposition_ts_df['i_MA_EPS'] / decomposition_ts_df['holding_i_EPS'] - 1
    decomposition_ts_df['rebalancing_NOISE_change'] = decomposition_ts_df['i_NOISE'] / decomposition_ts_df['holding_i_NOISE'] - 1
    decomposition_ts_df['rebalancing_PER_change'] = decomposition_ts_df['MA_PER'] / decomposition_ts_df['holding_i_MA_PER'] - 1
    decomposition_ts_df['rebalancing_noise_change'] = decomposition_ts_df['rebalancing_EARNINGS_change'] - decomposition_ts_df['rebalancing_MA_EARNINGS_change']
    ...
    annual_TR_log = np.log(decomposition_ts_df['i_amount'].iloc[-1] / decomposition_ts_df['i_amount'].iloc[0]) / n_freq * freq_scale
    annual_EPS_growth_total_log = np.log(decomposition_ts_df['i_EPS'].iloc[-1] / decomposition_ts_df['i_EPS'].iloc[0]) / n_freq * freq_scale
    annual_unrebalanced_PE_multipleExpansion_total_log = np.log(decomposition_ts_df['PER'].iloc[-1] / decomposition_ts_df['PER'].iloc[0]) / n_freq * freq_scale
    annual_MA_NOISE_growth_total_log = np.log(decomposition_ts_df['i_NOISE'].iloc[-1] / decomposition_ts_df['i_NOISE'].iloc[0]) / n_freq * freq_scale
    annual_MA_unrebalanced_PE_multipleExpansion_total_log = np.log(decomposition_ts_df['MA_PER'].iloc[-1] / decomposition_ts_df['MA_PER'].iloc[0]) / n_freq * freq_scale
    annual_holding_EPS_growth_log = np.log((decomposition_ts_df['holding_EARNINGS_change'] + 1).prod()) / n_freq * freq_scale
    annual_rebalancing_EPS_growth_log= np.log((decomposition_ts_df['rebalancing_EARNINGS_change'] + 1).prod()) / n_freq * freq_scale
    annual_rebalancing_MA_EPS_growth_log= np.log((decomposition_ts_df['rebalancing_MA_EARNINGS_change'] + 1).prod()) / n_freq * freq_scale
    annual_rebalancing_NOISE_growth_log = annual_rebalancing_EPS_growth_log - annual_rebalancing_MA_EPS_growth_log
    
    decomposition_sd = pd.Series({'TR_sd':np.log(decomposition_ts_df['i_amount'] / decomposition_ts_df['i_amount'].shift()).std(),
...
import time
start = time.time() 
test = get_factor_decomposition(univ_factor_df, topN_weight, all_infocode_roc_monthly, trading_cost, rebalancing_freq = 'm')
print("time :", time.time() - start)

decomposition_ts_df = test['decomposition_ts_df']
decomposition_ts_df.to_csv('new_henry.csv')
